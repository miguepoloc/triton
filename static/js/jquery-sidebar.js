...
$.fn.sidebarr = function(options) {
    ...
    $sidebarr.addClass('sidebarr-' + options.position);
    ...
    $tabs.children('li').children('a[href^="#"]').on('click', function(e) {
        ...
        if ($tab.hasClass('active'))
            ...
            $sidebarr.open(this.hash.slice(1), $tab);
    });

    $sidebarr.find('.sidebar-close').on('click', function() {
        $sidebarr.close();
    });
    ...
    $sidebarr.open = function(id, $tab) {
        ...
        $sidebarr.trigger('content', { 'id': id });

        if ($sidebarr.hasClass('collapsed')) {
            // open sidebar
            $sidebarr.trigger('opening');
            $sidebarr.removeClass('collapsed');
        }
    };
     ...
     * Close the sidebar (if necessary).
     */
    $sidebarr.close = function() {
        // remove old active highlights
        $tabs.children('li.active').removeClass('active');

        if (!$sidebarr.hasClass('collapsed')) {
            // close sidebar
            $sidebarr.trigger('closing');
            $sidebarr.addClass('collapsed');
        }
    };
